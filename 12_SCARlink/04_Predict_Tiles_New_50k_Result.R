...
result <- fread("/ix1/rduerr/shared/rduerr_wchen/Shiyue/Chen_Files/2023_07_DOGMA_Revision/SCARlink/Result/New_50k/gene_linked_tiles_celltype_updated.csv.gz")

result_f <- result %>% filter(FDR < 0.05)
write.xlsx(result_f, file = "/ix1/rduerr/shared/rduerr_wchen/Shiyue/Chen_Files/2023_07_DOGMA_Revision/SCARlink/Result/New_50k/gene_linked_tiles.xlsx")
...
saveRDS(colocal, "/ix1/rduerr/shared/rduerr_wchen/Shiyue/Chen_Files/2023_07_DOGMA_Revision/SCARlink/Result/New_50k/coloc_tile_p2g.rds")
...
colocal_s <- colocal %>% filter(gene == "PTGER4")
colocal_s <- colocal %>% filter(gene == "IL2RA")
colocal_s <- colocal %>% filter(gene == "CCL20")
colocal_s <- colocal %>% filter(gene == "CXCR5")

a <- result %>% select(gene, `Spearman corr`)
a <- a %>% distinct()

colocal <- readRDS("/ix1/rduerr/shared/rduerr_wchen/Shiyue/Chen_Files/2023_07_DOGMA_Revision/SCARlink/Result/New_50k/coloc_tile_p2g.rds")
library(openxlsx)
write.xlsx(colocal, file = "/ix1/rduerr/shared/rduerr_wchen/Shiyue/Chen_Files/2023_07_DOGMA_Revision/SCARlink/Result/New_50k/coloc_tile_p2g.xlsx")

# Table
table <- colocal %>% filter(gene == "PTGER4") %>%
  mutate(tile = paste0(chr, "-", start, "-", end))
table <- table %>% arrange(tile, peak_start, FDR)
write.xlsx(table, file = "/ix1/rduerr/shared/rduerr_wchen/Shiyue/Chen_Files/2023_07_DOGMA_Revision/SCARlink/Result/New_50k/tile_PTGER4.xlsx")

table <- colocal %>% filter(gene == "IL2RA") %>%
  mutate(tile = paste0(chr, "-", start, "-", end))
table <- table %>% arrange(tile, peak_start, FDR)
write.xlsx(table, file = "/ix1/rduerr/shared/rduerr_wchen/Shiyue/Chen_Files/2023_07_DOGMA_Revision/SCARlink/Result/New_50k/tile_IL2RA.xlsx")
