...
p1 <- DimPlot(data, reduction = 'wnn.umap', group.by = 'harmony_wnn_clusters', label = T, repel = TRUE, label.size = 4)
p2 <- DimPlot(data, reduction = 'rna.umap', group.by = 'harmony_rna_clusters', label = T, repel = TRUE, label.size = 4)
p3 <- DimPlot(data, reduction = 'adt.umap', group.by = 'harmony_adt_clusters', label = T, repel = TRUE, label.size = 4)
p4 <- DimPlot(data, reduction = 'atac.umap', group.by = 'harmony_atac_clusters', label = T, repel = TRUE, label.size = 4)

pdf('../plots/denoise/UMAP_all_original.pdf', width = 32, height = 8)
wrap_plots(p1, p2, p3, p4, ncol = 4)
dev.off()

load('../output/denoise/RNA_ADT.RData')

data[['RNA']] <- CreateAssayObject(counts = rna)
# data[['ADT']] <- CreateAssayObject(data = adt)
...
save(data, file = '../output/data_harmony_qc_tcell_updated_rna_denoise.RData')
...
pdf('../plots/denoise/UMAP_rna_denoise.pdf', width = 32, height = 8)
wrap_plots(p1, p2, p3, p4, ncol = 4)
dev.off()