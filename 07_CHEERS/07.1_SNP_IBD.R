...
write.csv(data_plot, file = '../output/SNP/enrich_immune_IBD.csv', col.names = T, row.names = F, quote = F)
...
pdf('../plots/SNP/enrich_immune_IBD.pdf', width = 8, height = 4)
...
pdf('../plots/SNP/enrich_immune_paired_IBD.pdf', width = 10, height = 5)
...
for(i in names(markerList)){
  ...
  for(j in c('snp_all', 'snp_lead', 'snp_p')){
    ...
    tmp <- rbind(perform_fisher_test_split(marker_peak, disease_snps, i, j, 'IBD'),
                 perform_fisher_test_split(GCcontrol_peak, disease_snps, i, j, 'IBD'),
                 perform_fisher_test_split(Distcontrol_peak, disease_snps, i, j, 'IBD'))
    ...
    tmp <- rbind(perform_fisher_test_split(marker_peak, disease_snps, i, j, 'UC'),
                 perform_fisher_test_split(GCcontrol_peak, disease_snps, i, j, 'UC'),
                 perform_fisher_test_split(Distcontrol_peak, disease_snps, i, j, 'UC'))
    tmp$order <- order(tmp$odds_ratio, decreasing = T)
    data_plot <- rbind(data_plot, tmp)
    ...
    tmp <- rbind(perform_fisher_test_split(marker_peak, disease_snps, i, j, 'CD'),
                 perform_fisher_test_split(GCcontrol_peak, disease_snps, i, j, 'CD'),
                 perform_fisher_test_split(Distcontrol_peak, disease_snps, i, j, 'CD'))
    tmp$order <- order(tmp$odds_ratio, decreasing = T)
    data_plot <- rbind(data_plot, tmp)
  }
}
...
write.csv(data_plot, file = '../output/SNP/enrich_disease_IBD.csv', col.names = T, row.names = F, quote = F)
...
p3 <- ggplot(data_plot3[data_plot3$peak == 'Cell type-specific chromatin accessible peaks',], 
  ...
  rremove('x.text')+ 
  coord_fixed(expand = T)
pdf('../plots/SNP/enrich_disease_IBD.pdf', width = 10, height = 7)
p3/p2/p1
dev.off()
...
p3 <- ggplot(data_plot3[data_plot3$peak == 'Cell type-specific chromatin accessible peaks',], 
  ...
  rremove('x.text')+ 
  coord_fixed(expand = T)
pdf('../plots/SNP/enrich_disease_IBD_updated.pdf', width = 10, height = 6.5)
p3/p2/p1
dev.off()
