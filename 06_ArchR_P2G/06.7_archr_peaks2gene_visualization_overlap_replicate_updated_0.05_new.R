...
PlotSNP_label <- function(snp_gr, peaks, region, color = "firebrick"){
  ...
  if (nrow(x = peak.df) > 0) {
    ...
    peak.plot <- ggplot(data = peak.df, aes(x = start - 100, y = 0, xend = end + 100, yend = 0, label = SNP, col = group)) + 
      facet_wrap(~name, strip.position = 'right') +
      geom_segment(size = 2, data = peak.df)+
      geom_text_repel(max.overlaps = Inf, min.segment.length = -1, angle = 90, nudge_y = 0.01, direction = 'x', segment.color = 'lightgrey', size = 1.5)+
      scale_color_manual(values = c('No SNP' = 'dodgerblue3', 'SNP' = 'firebrick'))+
      theme(legend.position = 'none')
  }
  else {
...
PlotSNP_label_norm <- function(snp_gr, peaks, region, color = "firebrick"){
  if (!inherits(x = region, what = "GRanges")) {
    region <- StringToGRanges(regions = region)
  }
  peak.intersect <- subsetByOverlaps(x = snp_gr, ranges = region)
  ...
  if (nrow(x = peak.df) > 0) {
    ...
    peak.plot <- ggplot(data = peak.df, aes(x = start - 100, y = 0, xend = end + 100, yend = 0, label = SNP, col = group)) + 
      facet_wrap(~name, strip.position = 'right') +
      geom_segment(size = 2, data = peak.df)+
      geom_text_repel(max.overlaps = Inf, min.segment.length = -1, angle = 90, nudge_y = 0.01, direction = 'x', segment.color = 'lightgrey')+
      scale_color_manual(values = c('No SNP' = 'dodgerblue3', 'SNP' = 'firebrick'))+
      theme(legend.position = 'none')
  }
  else {
...
modifyTrack_label_link_new <- function(p){
  ...
  #   theme(legend.position = 'none') + 
  #   theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))
  p11 <- p1 + labs(col = 'Correlation') + scale_colour_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) + theme(legend.position = 'left')
  p12 <- as_ggplot(get_legend(p11))
  p1 <- p1 + labs(col = 'Correlation') + scale_colour_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) + theme(legend.position = 'none', strip.text.y = element_text(hjust = 0))
...
expandRange = function(x, upstream=1000, downstream=1000) {
...
plotBrowserTrack_mod <- function (ArchRProj = NULL, region = NULL, groupBy = "Clusters", 
                                  ...
                                  title = "", verbose = TRUE, logFile = createLogFile("plotBrowserTrack")) 
{
  ...
  bulktrack <- ArchR:::.bulkTracks(ArchRProj = ArchRProj, 
                                   region = region[1], tileSize = tileSize, groupBy = groupBy, 
                                   ...
                                   title = title, useGroups = useGroups) + theme(plot.margin = unit(c(0.35, 
                                                                                                      0.75, 0.35, 0.75), "cm"))
  return(bulktrack)
}

modifyTrack_label_link_new_norm_sub <- function(p){
  p2 <- p
  ...
  p2 <- p2+theme(strip.text.y = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), 
                 strip.background = element_blank())+ggtitle('')+labs(y = '') + 
    theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))
  ...
  p6 <- PlotSNP_label_norm(snp_gr, peak_gr, region)
  p6 <- p6 +
    theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))
  
  p_new <- p6 + p5 + p4 + plot_spacer() + p2 + plot_spacer() +
    plot_layout(ncol = 1, heights = c(2.6, 0.2, 0.8, 0.2, 8, 10), widths = c(10))
  
  return(p_new)
}

modifyTrack_label_link_new_norm <- function(p){
  p1 <- p[[1]]$patches$plots[[4]]
  p2 <- p[[1]]$patches$plots[[1]]
  p3 <- p[[1]]$patches$plots[[2]]
...
snp_gr <- readRDS('../SNP/ibd_gr.RDS')

p2g_celltype_link <- list()
for(i in names(table(proj$celltype_id))){
  p2g_sub <- readRDS(paste0('DOGMA_filtered_multiome/', i, '/p2g_fdr_0.05_loop_replicate.RDS'))
...
# pdf('../../plots/visualization/Peak2Gene-Track_modify_label_PTGER4_overlap_link_0.05_replicate.pdf', width = 10, height = 10)
# p_new
# dev.off()

p_new <- modifyTrack_label_link_new(p)
pdf('../../plots/visualization/Peak2Gene-Track_modify_label_PTGER4_overlap_link_0.05_replicate_combined_new.pdf', width = 10, height = 10)
p_new
dev.off()

p <- plotBrowserTrack_mod(
  ArchRProj = proj, 
  groupBy = "celltype_id", 
  region = expandRange(StringToGRanges(regions = c('chr5-40490646-40491146')), 1000, 1000),
)

p_new <- modifyTrack_label_link_new_norm_sub(p)
pdf('../../plots/visualization/Peak2Gene-Track_modify_label_PTGER4_overlap_link_0.05_replicate_combined_new_1.pdf', width = 1.5, height = 10)
p_new
dev.off()
...
p <- plotBrowserTrack_mod(
  ArchRProj = proj, 
  groupBy = "celltype_id", 
  region = expandRange(StringToGRanges(regions = c('chr5-40486399-40486899')), 1000, 1000),
)

p_new <- modifyTrack_label_link_new_norm_sub(p)
pdf('../../plots/visualization/Peak2Gene-Track_modify_label_PTGER4_overlap_link_0.05_replicate_combined_new_2.pdf', width = 1.5, height = 10)
p_new
dev.off()
...
p <- plotBrowserTrack_mod(
  ArchRProj = proj, 
  groupBy = "celltype_id", 
  region = expandRange(StringToGRanges(regions = c('chr5-40578427-40578927')), 1000, 1000),
)

p_new <- modifyTrack_label_link_new_norm_sub(p)
pdf('../../plots/visualization/Peak2Gene-Track_modify_label_PTGER4_overlap_link_0.05_replicate_combined_new_3.pdf', width = 1.5, height = 10)
p_new
dev.off()
...
p_new <- modifyTrack_label_link_new(p)
pdf('../../plots/visualization/Peak2Gene-Track_modify_label_LNPEP_overlap_link_0.05_replicate_combined_new.pdf', width = 10, height = 10)
p_new
dev.off()
...
p <- plotBrowserTrack_mod(
  ArchRProj = proj, 
  groupBy = "celltype_id", 
  region = expandRange(StringToGRanges(regions = c('chr5-96896251-96896751')), 1000, 1000),
)

p_new <- modifyTrack_label_link_new_norm_sub(p)
pdf('../../plots/visualization/Peak2Gene-Track_modify_label_LNPEP_overlap_link_0.05_replicate_combined_new_1.pdf', width = 3, height = 10)
p_new
dev.off()
...
p_new <- modifyTrack_label_link_new(p)
pdf('../../plots/visualization/Peak2Gene-Track_modify_label_TNFRSF14_overlap_link_0.05_replicate_combined_new.pdf', width = 10, height = 10)
p_new
dev.off()
...
p_new <- modifyTrack_label_link_new_norm(p)
pdf('../../plots/visualization/Peak2Gene-Track_modify_label_IL2RA_overlap_link_0.05_replicate_combined_new.pdf', width = 10, height = 10)
p_new
dev.off()

snp_gr <- snp_gr[str_detect(snp_gr$Disease_merged, 'Lead'),]
...
p_new <- modifyTrack_label_link_new_norm(p)
pdf('../../plots/visualization/Peak2Gene-Track_modify_label_CCL20_overlap_link_0.05_replicate_combined_new.pdf', width = 10, height = 10)
p_new
dev.off()
...
p_new <- modifyTrack_label_link_new_norm(p)
pdf('../../plots/visualization/Peak2Gene-Track_modify_label_CXCR5_overlap_link_0.05_replicate_combined_new.pdf', width = 10, height = 10)
p_new
dev.off()
...
p_new <- modifyTrack_label_link_new_norm(p)
pdf('../../plots/visualization/Peak2Gene-Track_modify_label_ITGAL_overlap_link_0.05_replicate_combined_new.pdf', width = 10, height = 10)
p_new
dev.off()