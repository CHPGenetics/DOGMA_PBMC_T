...
peak <- read.table('../SNP/p2g_ibd_peak.bed')
...
snp <- read.table('../SNP/p2g_ibd_snp.bed')
colnames(snp)[4] <- 'SNP'
...
plotPDF(p, name = "Peak2Gene-Track_PTGER4", width = 12, height = 12, ArchRProj = proj, addDOC = FALSE)
...
p_new <- modifyTrack(p)
pdf('DOGMA_filtered_multiome/Plots/Peak2Gene-Track_modify_PTGER4.pdf', width = 12, height = 12)
p_new
dev.off()
...
modifyTrack_label <- function(p){
  ...
  p4 <- PlotAnnotation(annotations, region)
  p4 <- p4 + 
    theme(plot.margin = unit(c(0, 0, 0, 0), "cm"),strip.text.y = element_blank())
...
pdf('DOGMA_filtered_multiome/Plots/Peak2Gene-Track_modify_label_PTGER4_1.pdf', width = 3, height = 12)
p_new
dev.off()
...
p_new <- modifyTrack_label(p)
pdf('DOGMA_filtered_multiome/Plots/Peak2Gene-Track_modify_label_PTGER4_2.pdf', width = 3, height = 12)
p_new
dev.off()
