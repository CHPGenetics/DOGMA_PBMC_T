...
pdf('DOGMA_filtered_multiome/Plots/Peak2Gene-Track_modify_label_PTGER4_overlap.pdf', width = 10, height = 10)
p_new
dev.off()
...
pdf('DOGMA_filtered_multiome/Plots/Peak2Gene-Track_modify_label_PTGER4_overlap_link.pdf', width = 10, height = 10)
p_new
dev.off()
...
p_new <- modifyTrack_label_link(p)
pdf('DOGMA_filtered_multiome/Plots/Peak2Gene-Track_modify_label_PTGER4_overlap_link_0.01.pdf', width = 10, height = 10)
p_new
dev.off()
...
p_new <- modifyTrack_label_link(p)
pdf('DOGMA_filtered_multiome/Plots/Peak2Gene-Track_modify_label_PTGER4_overlap_link_0.001.pdf', width = 10, height = 10)
p_new
dev.off()

####################################
p <- plotBrowserTrack(
  ArchRProj = proj[proj$celltype_id %in% c('05_CD4+ Memory (Resting) - Th1', '06_CD4+ Memory (Activated) - Th1', '07_CD4+ Memory (Resting) - Th17', '08_CD4+ Memory (Activated) - Th17'),], 
  groupBy = "celltype_id", 
  geneSymbol = 'PTGER4', 
  plotSummary = c("bulkTrack", "loopTrack"),
  sizes = c(10, 3),
...
modifyTrack_label <- function(p){
  ...
  p_new <- plot_spacer() + p6 + plot_spacer() + p5 + plot_spacer() + p4 + p3 + p2 + 
    plot_layout(ncol = 2, heights = c(2.1, 0.2, 1, 3), widths = c(1,10))
  return(p_new)
}

p_new <- modifyTrack_label(p)
pdf('DOGMA_filtered_multiome/Plots/Peak2Gene-Track_modify_label_PTGER4_overlap_subset.pdf', width = 10, height = 4)
p_new
dev.off()
...
p2g_celltype_link <- p2g_celltype_link[5:8]
...
modifyTrack_label_link <- function(p){
  ...
  p_new <- p6 + p5 + p4 + p1 +
    plot_layout(ncol = 1, heights = c(2.1, 0.2, 1, 3))
  return(p_new)
}
...
p_new <- modifyTrack_label_link(p)
pdf('DOGMA_filtered_multiome/Plots/Peak2Gene-Track_modify_label_PTGER4_overlap_link_0.001_subset.pdf', width = 10, height = 3.4)
p_new
dev.off()
...
modifyTrack_label_link_new <- function(p){
  ...
  p1 <- p1 + labs(col = 'Correlation') + scale_color_gsea() + theme(legend.position = 'none')
  ...
  p_new <- plot_spacer() + p6 + plot_spacer() + p5 + plot_spacer() + p4 + p3 + p2 + p12 + p1 +
    plot_layout(ncol = 2, heights = c(2.2, 0.2, 1, 3, 3), widths = c(1,10))
  
  return(p_new)
}

p_new <- modifyTrack_label_link_new(p)
pdf('DOGMA_filtered_multiome/Plots/Peak2Gene-Track_modify_label_PTGER4_overlap_subset_combined.pdf', width = 10, height = 6)
p_new
dev.off()
