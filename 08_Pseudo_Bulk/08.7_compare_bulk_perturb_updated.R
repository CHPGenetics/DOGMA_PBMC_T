...
perturb=read.dir("~/RWorkSpace/GEO_data/DOGMA-seq/asap_reproducibility/CD4_CRISPR_asapseq/output/pseudo_bulk/updated/")
...
for(i in 1:5){
  ...
  pdf(paste0('../plots/pseudo_bulk/Bulk_Perturb/ATAC/', celltype, '_updated.pdf'), width = 7, height = 7)
  print(p)
  dev.off()
  assign(paste0('p',i), draw_plot_peak(celltype))
}

pdf(paste0('../plots/pseudo_bulk/Bulk_Perturb/ATAC/', 'Act_IL1B_IL23_PGE2_vs_Act_IL1B_IL23', '_updated.pdf'), width = 21, height = 14)
print(wrap_plots(p1, p2, p5, p3, p4, ncol = 3))
dev.off()

pdf(paste0('../plots/pseudo_bulk/Bulk_Perturb/ATAC/', 'Act_IL1B_IL23_PGE2_vs_Act_IL1B_IL23', '_activated_updated.pdf'), width = 21, height = 7)
print(wrap_plots(p1, p2, p5, ncol = 3))
dev.off()
