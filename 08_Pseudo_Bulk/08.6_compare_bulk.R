...
check_overlap_5 <- function(table1, table2){
  df1 <- table2$`CD4+ Memory (Resting) - Th1`
  df2 <- table2$`CD4+ Memory (Activated) - Th1`
  df3 <- table2$`CD4+ Memory (Resting) - Th17`
  df4 <- table2$`CD4+ Memory (Activated) - Th17`
  df5 <- table2$`CD4+ Regulatory (Activated)`
  x <- list(
    `Bulk RNA-seq` = table1[table1$Expression!='Not Significant',]$gene, 
    `CD4+ Memory (Resting) - Th1` = df1[df1$Expression!='Not Significant',]$gene,
    `CD4+ Memory (Activated) - Th1` = df2[df2$Expression!='Not Significant',]$gene,
    `CD4+ Memory (Resting) - Th17` = df3[df3$Expression!='Not Significant',]$gene,
    `CD4+ Memory (Activated) - Th17` = df4[df4$Expression!='Not Significant',]$gene,
    `CD4+ Regulatory (Activated)` = df5[df5$Expression!='Not Significant',]$gene
  )
  # p1 <- ggvenn(
  #   x, 
...
for(i in 1:20){
  ...
  pdf(paste0('../plots/pseudo_bulk/Bulk/RNA/', celltype, '.pdf'), width = 7, height = 7)
  print(p)
  dev.off()
  assign(paste0('p',i), draw_plot(celltype))
}

pdf(paste0('../plots/pseudo_bulk/Bulk/RNA/', 'Act_IL1B_IL23_PGE2_vs_Act_IL1B_IL23', '.pdf'), width = 35, height = 28)
...
for(i in 1:20){
  ...
  pdf(paste0('../plots/pseudo_bulk/Bulk/ATAC/', celltype, '.pdf'), width = 7, height = 7)
  print(p)
  dev.off()
  assign(paste0('p',i), draw_plot_peak(celltype))
}

pdf(paste0('../plots/pseudo_bulk/Bulk/ATAC/', 'Act_IL1B_IL23_PGE2_vs_Act_IL1B_IL23', '.pdf'), width = 35, height = 28)
print(wrap_plots(p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12, p13, p14, p15, p16, p17, p18, p19, p20, ncol = 5))
dev.off()
