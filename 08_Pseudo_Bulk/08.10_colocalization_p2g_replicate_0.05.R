...
read_pseudobulk = function(celltype){
  me = readRDS(paste0('../output/eQTA_updated/pseudobulk_eQTA_', celltype, '.RDS'))
  tmp = me$cis$eqtls
...
pdf('../plots/eQTA_replicate_0.05/pairs.pdf', width = 3, height = 5)
p1
dev.off()
...
p1 = ggplot(df, aes(x = genes, y = celltype, fill = celltype))+
  ...
  labs(y = 'Cell types', x = '# of linked genes', fill = '')+
...
pdf('../plots/eQTA_replicate_0.05/genes.pdf', width = 3, height = 5)
p1
dev.off()
...
p1 = ggplot(df, aes(x = peaks, y = celltype, fill = celltype))+
  ...
  labs(y = 'Cell types', x = '# of linked peaks', fill = '')+
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = 'none', axis.text.y = element_blank(), axis.ticks.y = element_blank())
pdf('../plots/eQTA_replicate_0.05/peaks.pdf', width = 3, height = 5)
p1
dev.off()
...
pdf('../plots/eQTA_replicate_0.05/shared_gene.pdf', width = 8, height = 5)
...
df = Reduce(rbind, lapply(levels[1:20], function(i){
  Reduce(rbind, lapply(levels[1:20], function(j){
      data.frame(x = i, y = j, z = check_overlap(j,i))
  }))
}))
df = df[df$z != 1,]
df$y = factor(df$y, levels = levels[20:1])
df$x = factor(df$x, levels = levels[1:20])

pdf('../plots/eQTA_replicate_0.05/shared_gene_updated.pdf', width = 8, height = 7.5)
ggplot(df, aes(x, y, fill = 100*z, label = round(100*z))) +
  geom_tile()+
  scale_fill_viridis(option = 'C', direction = -1)+
  geom_text()+
  labs(x = '', y = '', fill = '%')+
  theme_cowplot()+
  theme(axis.ticks = element_blank(), axis.line = element_blank(), axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
dev.off()
###############
...
pdf('../plots/eQTA_replicate_0.05/shared_peak.pdf', width = 8, height = 5)
Heatmap(mat*100, cluster_rows = F, cluster_columns = F, show_row_names = T, show_column_names = F, heatmap_legend_param = list(title = "%"), col = viridis(100, option = "C", direction = -1))
dev.off()
...
pdf('../plots/eQTA_replicate_0.05/shared_peak_updated.pdf', width = 8, height = 7.5)
ggplot(df, aes(x, y, fill = 100*z, label = round(100*z))) +
  geom_tile()+
  scale_fill_viridis(option = 'C', direction = -1)+
  geom_text()+
...
data_plot = data.frame(celltype = top_eQTA$celltype, num = top_eQTA$top_nearest, group = 'Top linked gene is the nearest gene')
data_plot = rbind(data_plot, data.frame(celltype = top_eQTA$celltype, num = top_eQTA$top_not_nearest, group = 'Top linked gene is not the nearest gene'))
data_plot$celltype = factor(data_plot$celltype, levels = levels)
data_plot$group = factor(data_plot$group, levels = c('Top linked gene is the nearest gene', 'Top linked gene is not the nearest gene'))
...
p1 = ggplot(data_plot, aes(x = num, y = celltype, fill = group))+
  ...
  theme_bw()+
  scale_fill_manual(values = c('Top linked gene is not the nearest gene' = 'firebrick', 'Top linked gene is the nearest gene' = 'dodgerblue3'))+
  scale_x_continuous(n.breaks = 10)+
  labs(y = 'Cell types', x = 'Proportions', fill = '')+
  theme(legend.position = 'bottom', axis.text.y = element_blank(), axis.ticks.y = element_blank())+
  guides(fill = guide_legend(ncol = 1, override.aes = list(size=4)))
pdf('../plots/eQTA_replicate_0.05/top_eQTA.pdf', width = 3, height = 5)
p1
dev.off()
...
for (i in 1:20){
  ...
  saveRDS(p2g, paste0('../output/ArchR/DOGMA_filtered_multiome/', name, '/p2g_fdr_0.05_loop_replicate.RDS'))
}
...
for (i in 1:20){
  ...
  saveRDS(p2g, paste0('../output/ArchR/DOGMA_filtered_multiome/', name, '/p2g_fdr_0.05_loop_replicate_peak_gene.RDS'))
}
