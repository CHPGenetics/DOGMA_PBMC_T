...
write.xlsx(da_snp_eqta_de_consistent, file = '../output/colocalization/peak_snp_p2g_gene_sig_consistent_updated.xlsx', overwrite = T)
da_snp_eqta_de_consistent <- da_snp_eqta_de_consistent[da_snp_eqta_de_consistent$gene_consistent_num > 0,]
write.xlsx(da_snp_eqta_de_consistent, file = '../output/colocalization/peak_snp_p2g_gene_sig_consistent_only_updated.xlsx', overwrite = T)
da_snp_eqta_de_consistent <- da_snp_eqta_de_consistent[!is.na(da_snp_eqta_de_consistent$snp_pos),]
write.xlsx(da_snp_eqta_de_consistent, file = '../output/colocalization/peak_snp_p2g_gene_sig_consistent_only_snp_updated.xlsx', overwrite = T)
...
bulk = read.xlsx('~/RWorkSpace/Duerr_bulk/eQTA/peak_snp_p2g_gene_sig_consistent_only_snp_updated.xlsx')

df$cell_type = factor(df$cell_type, levels = levels[levels %in% names(table(df$cell_type))])
df_celltype_pairs_list = lapply(names(table(df$cell_type)), function(celltype){
  df0 = df[df$cell_type == celltype,]
  df_pairs_list = lapply(1:nrow(df0), function(i){
    genes = str_split(df0$gene_consistent, '\\|')[[i]]
    pairs = paste(df0$peak[i], genes, sep = ':')
    return(pairs)
  })
  df_pairs = Reduce(c, df_pairs_list)
  return(data.frame(celltype = celltype, pairs = df_pairs))
})
df_celltype_pairs = Reduce(rbind, df_celltype_pairs_list)

bulk_pairs_list = lapply(1:nrow(bulk), function(i){
  genes = str_split(bulk$gene_consistent, '\\|')[[i]]
  pairs = paste(bulk$peak[i], genes, sep = ':')
  return(pairs)
})
bulk_pairs = Reduce(c, bulk_pairs_list)

df_celltype_pairs_overlap = df_celltype_pairs[df_celltype_pairs$pairs %in% bulk_pairs,]
df_celltype_pairs_overlap$peak = str_split_fixed(df_celltype_pairs_overlap$pairs, ':', 2)[,1]
df_celltype_pairs_overlap$gene = str_split_fixed(df_celltype_pairs_overlap$pairs, ':', 2)[,2]
write.xlsx(df_celltype_pairs_overlap, file = '../output/colocalization/peak_snp_p2g_gene_sig_consistent_only_snp_updated_overlap_bulk.xlsx', overwrite = T)
