...
ldsc_outpath=${PROJ_PATH}Results/02_S_LDSC/Output/Annotation_Peak2/
...
gs_file=${input_path}Peak2_list.txt
gc_file=${input_path}Peak2_coord_all.txt
...
###ldsc
ldsc_outpath=${PROJ_PATH}Results/02_S_LDSC/Output/Pseudo_Condition_DA/
...
gs_file=${input_path}Pseudo_Condition/Peak_list_${celltype}.txt
gc_file=${input_path}Peak2_coord_all.txt
b_ref=${pf_path}1000G.EUR.QC.${chr}
hmp_file=${hmp_path}hm.${chr}.snp
...
ldsc_outpath=${PROJ_PATH}Results/02_S_LDSC/Output/Pseudo_Condition_DA/Result/

python2 ${munge_sumstats} \
		--sumstats ${input_path}IBD.txt \
		--out ${ldsc_outpath}IBD \
		--chunksize 500000 \
		--N 59957 \
		--merge-alleles ${REF}w_hm3.snplist

python2 ${munge_sumstats} \
		--sumstats ${input_path}CD.txt \
		--out ${ldsc_outpath}CD \
		--chunksize 500000 \
		--N 40266 \
		--merge-alleles ${REF}w_hm3.snplist
