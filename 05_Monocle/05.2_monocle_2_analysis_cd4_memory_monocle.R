...
cds <- new_cell_data_set(assays(cds_sub_cut)[['counts']],
                        cell_metadata = colData(cds_sub_cut))
...
cds <- order_cells(cds, root_pr_nodes=get_earliest_principal_node(cds, 'CD4+ Memory (Resting) - Other'))
...
saveRDS(cds, file = '../output/monocle_2/cd4_memory_monocle/cds.RDS')
...
pdf('../plots/monocle_2/cd4_memory_monocle/UMAP.pdf', width = 24, height = 16)
